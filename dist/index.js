...
function nbsForm(player) {
    ...
    if (musics.length === 0) {
        player.sendModalForm(`${Aqua}${pluginName}`, `${Green}插件数据目录内还没有歌曲文件哦！赶快去寻找nbs音乐来播放吧！`, `知道了`, `知道了`, () => { });
        return;
    }
    const search = (param) => {
        ...
        form = form
            ...
            .setContent(`${Green}搜寻到 ${Yellow}${result.length} ${Green}条` +
    ...
    const sendForm = (page) => {
        ...
        form
            .setTitle(`${Aqua}${pluginName}`)
            .setContent(`${Green}页数 ${Yellow}${page} ${White}/ ${Gold}${maxPage} ${Gray}| ` +
        ...
        player.sendForm(form, (_, i) => {
            if (i !== null && i !== undefined) {
                ...
                if (i === 1) {
                    if (maxPage < 2) {
                        player.sendModalForm(`${Aqua}${pluginName}`, `${Red}页面总数小于2，无法跳转`, `知道了`, `知道了`, () => sendForm(page));
                        return;
                    }
                    const toPageForm = mc
                        .newCustomForm()
