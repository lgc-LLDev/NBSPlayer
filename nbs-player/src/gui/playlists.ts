    ...
    },
    {
      ...
      operation: async () => {
        if (await sendModalFormAsync(player, PLUGIN_NAME, '真的要删除吗？')) {
          const [data, playlistIndex] = await confManager.getByName(
            playListName
          );
          const { files } = data;
          const fileIndex = files.findIndex((x) => x === songFilename);
    ...
    .catch((err) => {
      if (err instanceof TipError)
        sendModalFormAsync(player, PLUGIN_NAME, err.message)
          .then(() =>
            playListSongForm(playListName, songFilename, player, parent)
          )
          .catch(logErr);
      else logErr(err);
    });
  ...
  new SimpleFormOperational(PLUGIN_NAME, name, [
    {
      ...
      operation: async () =>
        replacePlaylist(
          player,
          (await confManager.getByName(name))[0].files
        ).then(() => parent?.()),
    },
    {
      text: '查看',
    ...
    },
    {
      ...
      operation: async () => {
        ...
        const newFileList = data.files.filter((x) =>
          file.exists(`${NBS_PATH}/${x}`)
        );
...
export async function playListsForm(
  player: LLSE_Player,
  parent?: ParentFormFunc
) {
  ...
  playListOperateForm(res, player, () => playListsForm(player, parent)).catch(
    logErr
  );
}

export async function newListForm(
  player: LLSE_Player,
  parent?: ParentFormFunc
) {
